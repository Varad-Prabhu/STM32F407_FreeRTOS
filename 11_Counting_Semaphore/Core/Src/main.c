...
int main(void)
{
  ...
  /* USER CODE END 2 */
  eventSemaphore = xSemaphoreCreateCounting(5, 0);
  ...
  xTaskCreate(ConsumerTask, "Consumer", 128, NULL, 2, &consumerTaskHandle);
...
static void ConsumerTask(void *argument)
{
    ...
    for (;;)
    {
        /* Producer = 10 events/sec, Consumer = 1 event/sec */
        if (xSemaphoreTake(eventSemaphore, portMAX_DELAY) == pdTRUE)
        {
            len = snprintf(msg, sizeof(msg), "Event consumed\r\n");
